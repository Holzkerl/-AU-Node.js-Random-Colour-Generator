...
import * as boxSize from './boxsize.js';
...
const boxWidth = process.argv[4];
const boxHeight = process.argv[5];

if (hue === 'ask') {
  console.log(
    'write a color (red, green, blue, cyan, magenta or yellow) and brightness level (dark, bright or light)',
  );
} else {
  // declares colour variables and generates a random number between 0 and 16
  let red1 = rand(0, 15);
  let red2 = rand(0, 15);
  let green1 = rand(0, 15);
  let green2 = rand(0, 15);
  let blue1 = rand(0, 15);
  let blue2 = rand(0, 15);
...
###############################`); )
*/
  boxSize.box(boxWidth, boxHeight, number);
}
