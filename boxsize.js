...
export function box(boxWidth = 31, boxHeight = 9, number) {
  ...
  for (let count = (boxHeight - 3) / 2; count > 0; count--) {
    console.log(chalk.hex(number).bold(hashLine));
  }
  console.log(chalk.hex(number).bold(spaceLine));
  console.log(chalk.hex(number).bold(middleLine));
  console.log(chalk.hex(number).bold(spaceLine));
  for (let count = (boxHeight - 3) / 2; count > 0; count--) {
    console.log(chalk.hex(number).bold(hashLine));
  }
